...
extension CommentViewModel {
    func initialize<T: ModelProtocol>(_ receivedModel: T, completion: @escaping () -> Void) {
        guard let unwarp = receivedModel as? CommentModel else { unwarpError(); return }; model = unwarp
//        loadRealmData()
//        bindRealm()
        completion()
    }
...
final class CommentViewModel: BaseViewModel, ViewModelProtocol {
...
//    }
    ...
    enum Localization {
        static let title                     = "Comment.Title".localized()
    }
...
class CommentModel: ModelProtocol {
