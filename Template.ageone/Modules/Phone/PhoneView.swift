...
final class PhoneView: BaseController {
    
    // MARK: viewModel
    
    public var viewModel = PhoneViewModel()
    
    // MARK: ovveride
    
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.title = PhoneViewModel.Localization.title
    ...
    fileprivate let textField: MaterialTextField = {
        ...
        textField.placeholder = PhoneViewModel.Localization.placeholder
        return textField
    }()
    ...
    fileprivate let labelDescription: UILabel = {
        ...
        label.text = PhoneViewModel.Localization.description
        return label
    }()
    ...
    fileprivate let buttonNext: GradientButton = {
        ...
        button.setTitle(PhoneViewModel.Localization.button, for: UIControl.State.normal)
        button.alpha = 0.0
        return button
    }()
    ...
    fileprivate let labelAgreement: LabelActive = {
        ...
        label.numberOfLines = 0
        label.initialize(
            "\(PhoneViewModel.Localization.agreementText) \(PhoneViewModel.Localization.agreementButton)",
            "\(PhoneViewModel.Localization.agreementButton)",
...
extension PhoneView {
    ...
    fileprivate func renderUI() {
        ...
        textField.onTextEntered = { [unowned self] value in
            self.viewModel.model.phone = value
        }
...
//            self.emitEvent?(PhoneViewModel.EventType.onLogInButtonPressed.rawValue)
