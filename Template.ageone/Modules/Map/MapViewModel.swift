...
//        }
    ...
    public func riseUpPriceRequest() {
        let alert = alertAction.create("Увеличение стоимости", "Для более быстрой подачи автомобиля")
        alert.addAction(UIAlertAction(title: "Увеличить цену", style: UIAlertAction.Style.default, handler: { [unowned self] _ in
            self.riseUpPrice()
        }))
        alert.addAction(UIAlertAction(title: "Продолжить поиск", style: UIAlertAction.Style.default, handler: { [unowned self] _ in
