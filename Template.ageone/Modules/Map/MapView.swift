        ...
        case .destination:
            ...
            case (5 + rxData.order.value.waypoints.count - 1):
                ...
                cell?.button.onTap = { [unowned self] in
                    //                    log.verbose(rxData.order.value)
                    //                    if rxData.order.value.price == 0 {
                    //                        let from = rxData.order.value.from.street.isEmpty ? rxData.order.value.from.stringName : "\(rxData.order.value.from.street) \(rxData.order.value.from.home)"
                    //                        let to = rxData.order.value.to.street.isEmpty ? rxData.order.value.to.stringName : "\(rxData.order.value.to.street) \(rxData.order.value.to.home)"
                    //                        let alert = alertAction.create( "Ошибка базы данных заказчика", "\nc адреса: \(from)\nдо адреса: \(to)\n\nВаша база не вернула цену")
